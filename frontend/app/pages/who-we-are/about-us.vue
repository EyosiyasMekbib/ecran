<template>
  <main class="about-page">
    ...
    <!-- Mission / Vision / Legal / How We Work -->
    <section class="au-governance">
      <div class="au-governance-grid">
...
/* ── Hero ──────────────────────────────────────────────── */
.au-hero {
  padding: clamp(4rem, 10vw, 9rem) var(--gutter) 0;
  max-width: 1200px;
  margin: 0 auto;
}
...
.au-gov-card--accent {
  background: rgba(11, 143, 167, 0.15);
}

.au-gov-card:hover {
  background: rgba(11, 143, 167, 0.2);
}
...
.au-objectives-header {
  max-width: 56ch;
  margin-bottom: clamp(2.5rem, 5vw, 4rem);
}

.au-objectives-header h2 {
  font-family: var(--font-heading);
...
.au-theme-summary::-webkit-details-marker {
  display: none;
}
...
.au-theme-toggle::before,
.au-theme-toggle::after {
  content: '';
  position: absolute;
  background: var(--brand-dark);
  border-radius: 2px;
...
details[open] .au-theme-toggle::after {
  transform: rotate(90deg);
  opacity: 0;
}

details[open] .au-theme-toggle {
